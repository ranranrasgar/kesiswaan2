...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (strlen($_POST['nis']) > 20) {
		...
		$error_message = "Pekerjaan Ibu harus di isi!";
	...
	if (isset($_FILES['photo']) && $_FILES['photo']['error'] == 0) {
		...
		if (!in_array($file_extension, $allowed_extensions)) {
			$error_message = "Hanya file JPG, JPEG, PNG, JFIF dan GIF yang diizinkan!";
	...
	// If no validation errors, proceed with the insert
	if (empty($error_message)) {
		// Prepare SQL statement
		$stmt = $conn->prepare("INSERT INTO siswa 
        (nis, nama_siswa, jenis_kelamin, alamat, kelas, hobi, nama_ayah, pekerjaan_ayah, nama_ibu, pekerjaan_ibu, penghasilan_ayah, photo)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		...
		if (!$stmt) {
			...
			$stmt->bind_param(
				...
				$_POST['pekerjaan_ibu'],
				$penghasilan_ayah,
				$file_name // Ensure the 'penghasilan_ayah' is treated as a float
			);
			if (!$stmt->execute()) {
				// Display detailed error message
				$error_message = "Gagal menyimpan data: " . $stmt->error;
			} else {
				$error_message = 'Data berhasil di simpan!';
								...
								<div class="card mb-6">
									...
									<div class="card-body">
										<div class="d-flex align-items-start align-items-sm-center gap-6">
											<img src="../../assets/img/avatars/<?php echo isset($_POST['photo']) && !empty($_POST['photo']) ? $_POST['photo'] : '1.png'; ?>" alt="user-avatar"
												...
												<div class="col-md-6">
													...
													<div class="mb-3">
														<select class="form-select" id="jenis_kelamin" name="jenis_kelamin" required>
															...
															<option value="L"
																<?php echo (isset($_POST['jenis_kelamin']) && $_POST['jenis_kelamin'] == 'L') ? 'selected' : ''; ?>>L: Laki-Laki
															</option>
															<option value="P"
																<?php echo (isset($_POST['jenis_kelamin']) && $_POST['jenis_kelamin'] == 'P') ? 'selected' : ''; ?>>P: Perempuan
															</option>
														</select>
													</div>
														...
														<select class="form-select" id="kelas" name="kelas" required>
															...
															<?php while ($row = $TabelKelas->fetch_assoc()): ?>
																<option value="<?= $row['nama_kelas']; ?>"
																	<?= (isset($_POST['kelas']) && $_POST['kelas'] == $row['nama_kelas']) ? 'selected' : ''; ?>>
																	<?= $row['nama_kelas']; ?>
																</option>
															<?php endwhile; ?>
														</select>
													...
													<div class="form-floating form-floating-outline mb-3">
														<input class="form-control" type="text" id="hobi" name="hobi"
															value="<?php echo isset($_POST['hobi']) ? htmlspecialchars($_POST['hobi']) : ''; ?>"
												...
												<div class="col-md-6">
													<div class="form-floating form-floating-outline mb-3">
														<input class="form-control" type="text" id="nama_ayah"
															name="nama_ayah"
															value="<?php echo isset($_POST['nama_ayah']) ? htmlspecialchars($_POST['nama_ayah']) : ''; ?>"
														...
														<select class="form-select" id="pekerjaan_ayah" name="pekerjaan_ayah" required>
															...
															<?php while ($row = $TabelPekerjaan->fetch_assoc()): ?>
																<option value="<?= $row['nama_pekerjaan']; ?>"
																	<?= (isset($_POST['pekerjaan_ayah']) && $_POST['pekerjaan_ayah'] == $row['nama_pekerjaan']) ? 'selected' : ''; ?>>
																	<?= $row['nama_pekerjaan']; ?>
																</option>
															<?php endwhile; ?>
													...
													<div class="form-floating form-floating-outline mb-3">
														<input class="form-control" type="text" id="nama_ibu"
															name="nama_ibu"
															value="<?php echo isset($_POST['nama_ibu']) ? htmlspecialchars($_POST['nama_ibu']) : ''; ?>"
													...
													<div class="form-floating form-floating-outline mb-3">
														<select class="form-select" id="pekerjaan_ibu" name="pekerjaan_ibu" required>
															<option value="">-- Pilih Pekerjaan --</option>
															<?php $TabelPekerjaan->data_seek(0);
															while ($row = $TabelPekerjaan->fetch_assoc()): ?>
																<option value="<?= $row['nama_pekerjaan']; ?>"
																	<?= (isset($_POST['pekerjaan_ibu']) && $_POST['pekerjaan_ibu'] == $row['nama_pekerjaan']) ? 'selected' : ''; ?>>
																	<?= $row['nama_pekerjaan']; ?>
																</option>
															<?php endwhile; ?>
													...
													<div class="form-floating form-floating-outline mb-3">
														<input class="form-control" type="text" id="penghasilan_ayah"
															...
															value="<?php echo isset($_POST['penghasilan_ayah']) ? htmlspecialchars($_POST['penghasilan_ayah']) : ''; ?>"
